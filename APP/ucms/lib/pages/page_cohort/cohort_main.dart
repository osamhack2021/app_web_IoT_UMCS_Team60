...
class CohortMain extends StatefulWidget {
  CohortMain({Key? key, this.location, this.state, this.positions, this.times})
      : super(key: key);
...
class _CohortMainState extends State<CohortMain> {
  ...
  void initState() {
    ...
    expanItemsTimes=
        List<ExpanItem>.generate(widget.times!.length, (index) {
      return ExpanItem(expanded: false, header: widget.times![index].place.name, body : widget.times![index].toListTiles());
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> widgetOptions =_buildPages(c, positions : widget.positions!, expanItemsPos : expanItemsPos,
              times : widget.times!, expanItemsTimes: expanItemsPos, 
        ...
        child: RefreshIndicator(
          ...
          onRefresh: () async {
            ...
            setState(() async {
              ...
              widgetOptions = _buildPages(c, positions : widget.positions!, expanItemsPos : expanItemsPos,
              times : widget.times!, expanItemsTimes: expanItemsPos, 
         assembleVisible : assembleVisible,  name : name,  nameCon : nameCon , formKey : formKey,  
         rankCon : rankCon,  timeCon : timeCon,  tempCon : tempCon ,  descCon : descCon);
            });
  ...
  List<Widget> _buildPages(CohortController c, {required List<PositionList> positions, required List<ExpanItem> expanItemsPos, 
        required List<TimeList> times, required List<ExpanItem> expanItemsTimes,
