...
router.post('/', managerAuth.checkLogin, async (req, res) => {
    ...
    try {
        var sql = "INSERT INTO doom VALUES (NULL, ?, ?, 0)";
        var [results] = await dbPromiseConnection.query(sql, [req.body.name, req.body.beacon_id]);
        
        sql = "UPDATE beacon SET doom_id=? WHERE id=?";
...
router.delete('/:id', managerAuth.checkLogin, async (req, res) => {
    ...
    var sql = "UPDATE beacon SET doom_id=NULL WHERE doom_id=?";
    await dbPromiseConnection.query(sql, [req.params.id]);

    sql = "DELETE FROM doom WHERE id=?";
    dbConnection.query(sql, [req.params.id], (err, rows) => {
        if(err)
            return res.status(400).json({
                code: 4,
